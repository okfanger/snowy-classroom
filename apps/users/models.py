...
class PermissionAction(FactSchema):
    permission = models.ForeignKey('Permission', related_name='actionEntitySet', on_delete=models.CASCADE, verbose_name='权限子集')
...
class MenuRouter(FactSchema):
    ...
    hideChildrenInMenu= models.BooleanField(verbose_name='强制菜单显示为Item而不是SubItem(配合 meta.hidden)', default=False)
...
class MenuRouterMeta(FactSchema):
    ...
    hidden = models.BooleanField(verbose_name='配合hideChildrenInMenu使用，用于隐藏菜单时，提供递归到父菜单显示选中菜单项_（可参考个人页配置方式）', default=False)
    hiddenHeaderContent = models.BooleanField(verbose_name='隐藏PageHeader组件中的页面带的面包屑和页面标题栏', default=False)
