...
teacher_role_routes = [
    ...
    },
    {
        ...
        'id': 51004,  # 路由ID
        "path": "sendHomeWork",  # 路由地址（显示在浏览器URL上的）
        'meta': {  # 元信息
    ...
    {
        'name': 'correctionHomework',  # 路由名
        'parentId': 51003,  # 父菜单ID，如果没有父菜单就写0
        'id': 51011,  # 路由ID
        "path": "correctionHomework",  # 路由地址（显示在浏览器URL上的）
        'meta': {  # 元信息
            'title': 'menu.correctionHomework',  # 国际化标识，可以在前端locales里增加
            'icon': 'book',  # 图标，这个可以从Ant Design Vue 组件库中找
            'show': True,  # 是否显示
        },
        'component': 'teacher/course/correctionHomework.vue',  # 前端.vue文件的相对地址（views为根目录，最前面不用写/）
        'redirect': ''  # 重定向，如果这边填了其实component就没啥用，不需要重定向就留空字符串即可
    },
    {
        'name': 'correctionDetail',  # 路由名
        'parentId': 51003,  # 父菜单ID，如果没有父菜单就写0
        'id': 51012,  # 路由ID
        "path": "correctionDetail",  # 路由地址（显示在浏览器URL上的）
        'meta': {  # 元信息
            'title': 'menu.correctionDetail',  # 国际化标识，可以在前端locales里增加
            'icon': 'book',  # 图标，这个可以从Ant Design Vue 组件库中找
            'show': False,  # 是否显示
        },
        'component': 'teacher/course/correctionDetail.vue',  # 前端.vue文件的相对地址（views为根目录，最前面不用写/）
        'redirect': ''  # 重定向，如果这边填了其实component就没啥用，不需要重定向就留空字符串即可
    },
    {
        'name': 'classroom',  # 路由名
    ...
    },
{
    ...
    },
    {
        ...
        'name': 'examPreview',  # 路由名
        'parentId': 51046,  # 父菜单ID，如果没有父菜单就写0
        'id': 51053,  # 路由ID
        "path": "preview",  # 路由地址（显示在浏览器URL上的）
        'meta': {  # 元信息
            'title': 'menu.exam.preview',  # 国际化标识，可以在前端locales里增加
            'icon': 'profile',  # 图标，这个可以从Ant Design Vue 组件库中找
            'show': False,  # 是否显示
            # 'hidden': True
        },
        'component': 'teacher/exam/preview.vue',  # 前端.vue文件的相对地址（views为根目录，最前面不用写/）
        'redirect': ''  # 重定向，如果这边填了其实component就没啥用，不需要重定向就留空字符串即可
